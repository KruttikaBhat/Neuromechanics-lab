...
for i in range(8):
    ...
    popt,pcov=scipy.optimize.curve_fit(lambda t,a,b: a*np.exp(b*t),  x,  y) #for the exponential fit
    #print(popt)
    a=popt[0]
    b=popt[1]
    ...
    plt.savefig('correlation_1/Performance/PerDay/avgTransitionsPerson'+str(i+1))
...
plt.plot(x,func(x,*popt),label='Fitted curve top',color='b') #for the exponential fit
plt.text(4,0.2,'top 4 = '+str.format('{0:.4f}', popt[0])+'*exp('+str.format('{0:.4f}', popt[1])+'x)',fontsize=10) #for the exponential fit
...
plt.savefig("correlation_1/Performance/PerDay/avgTransitionsAllSubjects0") #for the exponential fit
#plt.show()
